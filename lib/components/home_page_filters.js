...
class HomePageFilters extends ComponentBase {
  ...
  async addLocation() {
    ...
    try {
      ...
      this.log(
        `Location ${this.location} was not visible in drop down list. Continue with text input.`,
      );
    }
  }
  ...
  async checkIfCalendarIsVisible(currentDate) {
    this.log("Check if calendar is visible");
    await this.waitForElementById("tabs");
    let currentMonthString = getStringMonthFromDate(currentDate);
  ...
  async moveArrowToPick(currentDate, dateToPick) {
    ...
    await this.checkIfCalendarIsVisible(currentDate);
  }
  ...
  async addIntervalToDatePicker() {
    ...
    await this.checkIfCalendarIsVisible(this.currentDate);
