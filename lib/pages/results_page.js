...
class ResultsPage extends BasePage {
  ...
  async checkNumberOfBeds() {
    await this.resultsSection.checkNumberOfBeds(
  ...
  async checkMapSection() {
    ...
    let propertyName = await this.resultsSection.getCardIndexName(
      cards,
      0
    );
    let propertyTitle = await this.resultsSection.getCardIndexTitle(
      cards,
      0
    );
    let prices = await this.driver.findElements(
      By.xpath(
        "//div[@data-testid='price-availability-row']//span//span[2]"
      )
    );
    ...
    await this.mapComponent.clickOnBulletFromMap(
      propertyTitle,
      propertyPrice
    );
